...
export const api = {
  ...
  ranks: () => request<string[]>("/api/officers/ranks"),
  uploadOfficers: (form: FormData) => request<{ imported: number; skipped_no_belt: number; skipped_duplicate: number; total: number }>("/api/officers/upload", { method: "POST", body: form }),
