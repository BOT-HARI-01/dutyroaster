...
export async function POST(req: NextRequest) {
  ...
  ws.eachRow((row, rowNum) => {
    ...
    const belt = getVal(cBelt);
    if (!belt) { skippedNoBelt++; return; }
  ...
  await prisma.uploadedFile.create({ data: { filename: file.name, file_type: "officers_excel", row_count: imported } });
  return Response.json({ imported, skipped_no_belt: skippedNoBelt, skipped_duplicate: skippedDuplicate, total: rows.length + skippedNoBelt });
}
